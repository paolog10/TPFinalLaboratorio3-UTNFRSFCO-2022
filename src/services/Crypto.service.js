...
export default new class CryptoService {
  async getCrypto() {
    ...
    for (let index = 0; index < data.length; index++) {
      try {
        const currentPrice = await this.getCriptoYa(data[index].symbol.toUpperCase(), 1);
        let arrayCurrenPrice = Object.entries(currentPrice).map( elem => { return { ...elem[1], name: elem[0] }} );
